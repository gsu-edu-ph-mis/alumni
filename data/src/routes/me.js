...
router.get('/me/update-alumni-record/basic-info', middlewares.guardRoute(['read_own_record']), async (req, res, next) => {
    try {
        let sessionUser = res.locals.user;
        ...
        res.render('me/update-alumni-record-basic-info.html', data);
    } catch (err) {
        next(err);
    }
});
// B.2 Submit Update Alumni Records - Basic Info.
router.patch('/me/update-alumni-record/:almId/basic-info', middlewares.antiCsrfCheck, middlewares.guardRoute(['update_own_record']), middlewares.getAlm(), async (req, res, next) => {
    try {
        let editAlm = res.alm
        ...
        res.redirect('/me/update-alumni-record/education')
    } catch (err) {
        console.error(err)
        flash.error(req, 'basic_info', err.message);
        return res.redirect('/me/update-alumni-record/basic-info')
    }
});
// B.3 Display Update Alumni Records - Education.
router.get('/me/update-alumni-record/education', middlewares.guardRoute(['read_own_record']), async (req, res, next) => {
    try {
        let sessionUser = res.locals.user;
        ...
        res.render('me/update-alumni-record-education.html', data);
    } catch (err) {
        next(err);
    }
});
// B.4 Submit Update Alumni Records - Education
router.patch('/me/update-alumni-record/:almId/education', middlewares.antiCsrfCheck, middlewares.guardRoute(['update_own_record']), middlewares.getAlm(), async (req, res, next) => {
    try {
        let editAlm = res.alm
        let payload = JSON.parse(req?.body?.payload)
        console.log(payload)
        ...
        flash.ok(req, 'work', 'Updated the educational records. Lastly, please update the work details.')
        res.redirect('/me/update-alumni-record/work')
    } catch (err) {
        console.error(err)
        flash.error(req, 'educ', err.message);
        return res.redirect('/me/update-alumni-record/education')
    }
});
// B.5 Display Update Alumni Records - Work.
router.get('/me/update-alumni-record/work', middlewares.guardRoute(['read_own_record']), async (req, res, next) => {
    try {
        let sessionUser = res.locals.user;
        ...
        res.render('me/update-alumni-record-work.html', data);
    } catch (err) {
        next(err);
    }
});
// B.6 Submit Update Alumni Records - Work
router.patch('/me/update-alumni-record/:almId/work', middlewares.antiCsrfCheck, middlewares.guardRoute(['update_own_record']), middlewares.getAlm(), async (req, res, next) => {
    try {
        let editAlm = res.alm
        let payload = JSON.parse(req?.body?.payload)
        console.log(payload)
        ...
        flash.ok(req, 'home', 'Updated the work details. Congratulations! Your alumni record is up to date.')
        res.redirect(`/me`)
    } catch (err) {
        console.error(err)
        flash.error(req, 'work', err.message);
        return res.redirect('/me/update-alumni-record/work')
    }
});
...
router.get('/me/data-privacy', middlewares.guardRoute(['read_own_record']), async (req, res, next) => {
    try {        
        res.render('me/data-privacy.html');
    } catch (err) {
        next(err);
    }
});
